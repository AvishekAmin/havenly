...
function generateReviewComment(listing) {
  ...
  let sentences = selectedPools.map(
  ...
  if (destKey && Math.random() < 0.4) {
    ...
    const destSentence =
      destSentences[Math.floor(Math.random() * destSentences.length)];
  ...
  if (wordCount < 40) {
    ...
    if (remainingPools.length > 0) {
      const extraSent =
        remainingPools[0][Math.floor(Math.random() * remainingPools[0].length)];
...
async function seedReviews() {
  try {
    ...
    for (let listing of listings) {
      ...
      console.log(
        `Generating ${numReviews} reviews for: "${listing.title}"...`,
      );
    ...
    console.log(
      `Seeding complete! Successfully generated and linked ${totalReviewsCreated} reviews.`,
    );
