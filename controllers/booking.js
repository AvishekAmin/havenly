...
module.exports.createOrder = wrapAsync(async (req, res) => {
  ...
  if (listing.owner && listing.owner.equals(req.user._id)) {
    return res.status(400).json({
      success: false,
      message:
        "As the host of this property, you cannot book your own listing.",
    });
  }
  ...
  try {
    bookingSummary = calculateBookingPrice(listing.price, checkIn, checkOut);
  } catch (error) {
    return res.status(400).json({
      success: false,
...
module.exports.verifyPayment = wrapAsync(async (req, res) => {
  ...
  const { checkInDate, checkOutDate, numberOfNights, basePrice, totalPrice } =
    bookingSummary;
  ...
  sendBookingConfirmationEmail({
    ...
    console.error(
      "Non-fatal error: Booking confirmation email failed to send:",
      err.message,
    );
  });
